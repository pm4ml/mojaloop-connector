...
const { MetricsServer, MetricsClient } = require('@internal/metrics');
...
const LOG_ID = {
    ...
    CONTROL:   { app: 'mojaloop-connector-control-server' },
...
class Server extends EventEmitter {
    ...
    async start() {
        ...
        this.controlServer.on(ControlServer.EVENT.RECONFIGURE, this.restart.bind(this));
    }
    ...
    async restart(newConf) {
        ...
        assert(newConf.inbound.port === this.conf.inbound.port
            ...
            && newConf.control.port === this.conf.control.port,
        ...
        const [restartInboundServer, restartOutboundServer, restartControlServer] = confChanged
            ? await Promise.all([
                ...
                this.controlServer.reconfigure({
                    logger: this.logger.push(LOG_ID.CONTROL),
        ...
        // file.
        [restartTestServer, restartOAuthTestServer, restartInboundServer, restartOutboundServer, restartControlServer]
        ...
        await Promise.all([
            ...
            this.controlServer.notifyClientsOfCurrentConfig(),
        ]);
    }
